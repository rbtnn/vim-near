...
function! dig#sillyiconv#utf_8(input) abort
...
function! dig#sillyiconv#euc_jp(input) abort
...
function! dig#sillyiconv#shift_jis(input) abort
...
function! dig#sillyiconv#iso_2022_jp(input) abort
...
function! dig#sillyiconv#of(input) abort
	if dig#sillyiconv#iso_2022_jp(a:input)
		return "iso-2022-jp"
	elseif dig#sillyiconv#utf_8(a:input)
		return "utf-8"
	elseif dig#sillyiconv#euc_jp(a:input)
		return "euc-jp"
	elseif dig#sillyiconv#shift_jis(a:input)
		return "shift_jis"
	else
...
function! dig#sillyiconv#iconv_one_nothrow(val) abort
	let val = a:val
	try
		let val = dig#sillyiconv#iconv_one(val)
	catch
	endtry
	return val
endfunction

function! dig#sillyiconv#iconv_one(val) abort
	return get(dig#sillyiconv#iconv([(a:val)]), 0, '')
endfunction

function! dig#sillyiconv#iconv(lines, ...) abort
	let to_encode = a:0 > 0 ? a:1 : &encoding
	let lines = copy(a:lines)
	for i in range(0, len(lines) - 1)
		if 0 < len(filter(split(lines[i], '\zs'), {i,x -> 0x80 < char2nr(x) }))
			let lines[i] = iconv(lines[i], dig#sillyiconv#of(lines[i]), to_encode)
		endif
	endfor
...
function! dig#sillyiconv#system(cmd) abort
	return join(dig#sillyiconv#iconv(split(system(a:cmd), "\n", 1)), "\n")
endfunction

function! dig#sillyiconv#qficonv() abort
	let xs = getqflist()
	for x in xs
		if has_key(x, 'text')
			let x['text'] = dig#sillyiconv#iconv_one_nothrow(x['text'])
		endif
	endfor
	call setqflist(xs)
endfunction
