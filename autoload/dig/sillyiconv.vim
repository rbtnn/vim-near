...
function dig#sillyiconv#utf_8(input)
...
function dig#sillyiconv#euc_jp(input)
...
function dig#sillyiconv#shift_jis(input)
...
function dig#sillyiconv#iso_2022_jp(input)
...
function dig#sillyiconv#of(input)
...
function dig#sillyiconv#iconv_one_nothrow(val)
  let val = a:val
  try
...
function dig#sillyiconv#iconv_one(val)
  return get(dig#sillyiconv#iconv([(a:val)]), 0, '')
endfunction

function dig#sillyiconv#iconv(lines, ...)
...
function dig#sillyiconv#system(cmd)
  return join(dig#sillyiconv#iconv(split(system(a:cmd), "\n", 1)), "\n")
endfunction

function dig#sillyiconv#qficonv()
  let xs = getqflist()
  for x in xs
    if has_key(x, 'text')
